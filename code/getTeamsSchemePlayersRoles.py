...
df_roles = pd.merge(
    pd.DataFrame(players)[["wyId", "shortName", "currentTeamId"]],
    getMainRole(matchFormations),
    left_on="wyId",
    right_index=True,
)
df_roles.columns = ["player_id", "player_name", "team_id", "main_role"]
df_roles["main_role"] = df_roles.main_role.apply(lambda x: ROLES[x])
