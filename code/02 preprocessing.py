...
def getMainTeamFromChildren(children_team_id, teams):
    for team in teams:
        for team_info in team["children"]:
            if team_info["wyId"] == children_team_id:
                return team["name"]
    return "N/A"
...
df_roles["main_role"] = df_roles.main_role.apply(lambda x: ROLES[x] if x == x else "N/A")
...
df["foot"] = df.foot.apply(lambda x: "N/A" if x is None else x)
df["contractExpiration"] = df.contractExpiration.apply(lambda x: int(x[:4]) if x is not None and x == x else -1)

for idx, row in df.iterrows():
    if row.team_name != row.team_name:
        team_name = getMainTeamFromChildren(row.team_id, teams)
        if team_name == team_name:
            df.loc[idx, "team_name"] = team_name
            df.loc[idx, "main_scheme"] = df[df.team_name == team_name].iloc[0].main_scheme if team_name != "N/A" else "N/A"
